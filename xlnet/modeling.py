...
class RelativeAttention(tf.keras.layers.Layer):
    ...
    def __call__(self, q_head, k_head_h, v_head_h, k_head_r, seg_embed, seg_mat,r_w_bias, r_r_bias, r_s_bias, attn_mask):
        inputs = pack_inputs([
...
class PositionwiseFF(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.inner_dropout = tf.keras.layers.Dropout(rate=self.dropout, name='drop_1')
...
class RelativeMultiheadAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        Implements call() for the layer.
        """
        (h, r, r_w_bias, r_r_bias, seg_mat, r_s_bias, seg_embed, attn_mask, mems) = unpack_inputs(inputs)
...
class TransformerXLModel(tf.keras.layers.Layer):
    ...
    def __call__(self,
                 ...
                 target_mapping=None,
                 inp_q=None):
        # Uses dict to feed inputs into call() in order to keep mems as a python list.
